...
>2. Echarts的基本使用步骤
>
>3. 独立使用echarts完成可视化项目的开发

### 什么是数据可视化

**什么是数据可视化**

1. 主要目的：借助于图形化的手段，清晰有效的传达与沟通信息

2. 数字非常不直观，把数字转换成图形，揭示蕴含在数据种的规律和道理
...
1. 市场需求：应对现在数据可视化的趋势，越来越多企业需要在很多场景(营销数据，生产数据，用户数据)下使用，可视化图表来展示体现数据，让数据更加直观，数据特点更加突出。我们引入 '立可得' 数据可视化项目。

2. 学习阶段需求：承上启下作用
   * 复习以前内容，布局相关，js/jq相关

   * 学习服务器端做铺垫，如何把服务器端数据渲染到页面

​	该项目除了使用了基础的DIV+CSS布局，还引入了一些C3技术，还引入了各类图表的绘制，以及高级的地图数据可视化案例。主要功能有：饼状图、柱状图、线形图、地图 ...

![1576208177581](http://weldon.flywen.top/image/123.png)
...
- js 400行 (有效)
  ...
  ![CleanShot 2020-08-04 at 11.39.44@2x](http://weldon.flywen.top/image/CleanShot%202020-08-04%20at%2011.39.44@2x.png)
...
- legend：图例组件
- toolbox: 工具栏
- <mark>grid</mark>：直角坐标系内绘图网格。 调节绘图区域和图表容器之间间距,left,right,bottom,containLabel
- <mark>xAxis</mark>：直角坐标系 grid 中的 x 轴
  -  boundaryGap: 坐标轴两边留白策略 true，这时候刻度只是作为分隔线，标签和数据点都会在两个刻度之间的带(band)中间。
- yAxis：直角坐标系 grid 中的 y 轴
- color：调色盘颜色列表，值为数组

- <mark>series</mark>：值为数组
...
- **最终方案**
  ...
  * flexible.js(检测浏览器宽度，根据浏览器宽度修改html文字大小，修改html大小)。
* rem单位(cssRem插件)取决于html文字大小
  * flex布局(页面快速布局)

- <mark>适配方案细则</mark>

  1. flexible.js 把屏幕分为 24 等份(源码10等分，手动修改)
...
1. 引入文件(jquery.js flexible.js  echarts.js index.css)
2. 自动刷新插件

### 07-基础布局(body和viewport制作)
...
![CleanShot 2020-08-04 at 16.51.53@2x](http://weldon.flywen.top/image/CleanShot%202020-08-04%20at%2016.51.53@2x.png)

### 08-边框图片(border-image)
...
![1576483576664](http://weldon.flywen.top/image/1576483576664.png)
...
- 边框图片资源地址
- 裁剪尺寸（上 右 下 左）单位默认px，可使用百分百(不能加单位)。
- 边框图片的宽度，默认边框的宽度。
- 平铺方式：
  - stretch 拉伸（默认）
...
![](http://weldon.flywen.top/image/qg.png)

**<mark>其他面板样式</mark>**

- 面板 .panel 
  - 容器 .inner   内边距是 上下24px  左右 36px
  - top/left/right/bottom采用负距离的方式进行布局
...
**<mark>通过类名调用字体图标</mark>**

1. 引入包含字体图片的css文件

2. 直接调用类名开始设置图标即可(类名在css文件中标出)
...
**<mark>tooltip含义解析</mark>**

**<mark>series含义解析</mark>**
...
<img src="http://weldon.flywen.top/image/CleanShot%202020-08-06%20at%2015.58.08@2x.png" alt="CleanShot 2020-08-06 at 15.58.08@2x" style="zoom:50%;" />

- **<mark>需求6：调整数据，与省略图形定制</mark>**
...
**需求<mark>4： 雷达图 坐标轴轴线相关设置(竖线) axisLine</mark>** 
...
**<mark>需求7： 标记的图形(拐点）设置  注意 series 里面设置</mark>**
...
<img src="http://weldon.flywen.top/image/CleanShot%202020-08-07%20at%2016.12.06@2x.png" alt="CleanShot 2020-08-07 at 16.12.06@2x" style="zoom:30%;" />
...
![1576664444951](http://weldon.flywen.top/image/1576664444951.png)
